 ...
 * Signature: (II)V
 */
void JNICALL Java_jpigpio_Pigpio_gpioSetMode(JNIEnv *env, jobject obj,
		jint gpio, jint mode) {
 ...
 * Signature: (I)I
 */
jint JNICALL Java_jpigpio_Pigpio_gpioGetMode(JNIEnv *env, jobject obj,
		jint gpio) {
 ...
 * Signature: (II)V
 */
void JNICALL Java_jpigpio_Pigpio_gpioSetPullUpDown(JNIEnv *env, jobject obj,
		jint pin, jint pud) {
 ...
 * Signature: (IZ)V
 */
void JNICALL Java_jpigpio_Pigpio_gpioWrite(JNIEnv *env, jobject obj, jint gpio,
		jboolean value) {
 ...
 * Signature: (II)V
 */
void JNICALL Java_jpigpio_Pigpio_gpioServo(JNIEnv *env, jobject obj, jint gpio,
		jint pulseWidth) {
 ...
 * Signature: (IJZ)V
 */
void JNICALL Java_jpigpio_Pigpio_gpioTrigger(JNIEnv *env, jobject obj,
		jint gpio, jlong pulseLen, jboolean level) {
 ...
 * Signature: (I[B)V
 */
void JNICALL Java_jpigpio_Pigpio_i2cWriteDevice(JNIEnv *env, jobject obj,
		jint handle, jbyteArray array) {
	char *buf;
	unsigned count;
 ...
 * Signature: (J)V
 */
void JNICALL Java_jpigpio_Pigpio_gpioDelay(JNIEnv *env, jobject obj,
		jlong delay) {
 ...
 */
void JNICALL Java_jpigpio_Pigpio_gpioSetAlertFunc(JNIEnv *env, jobject obj,
		jint gpio, jobject alert) {
...
void alertCallback(int gpio, int level, unsigned int tick) {
	...
	if (alertFunctions[gpio] == NULL) {
		printf(
				"JPigpio: alertCallback(gpio=%d, level=%d, tick=%d): Odd ... alert callback but no alert function registered\n",
				gpio, level, tick);
		return;
	}
	...
	(*env)->CallVoidMethod(env, alertFunctions[gpio], methodId, gpio, level,
			(jlong) tick);
 ...
 */
jthrowable createPigpioException(JNIEnv *env, int rc) {
	jclass pigpioExceptionClass = (*env)->FindClass(env,
			"jpigpio/PigpioException");
	jmethodID pigpioExceptionConstructorId = (*env)->GetMethodID(env,
			pigpioExceptionClass, "<init>", "(I)V");
	jthrowable newPigpioException = (*env)->NewObject(env, pigpioExceptionClass,
			pigpioExceptionConstructorId, rc);
